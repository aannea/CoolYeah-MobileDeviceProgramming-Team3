...
class LaporanSosial extends StatefulWidget {
  String? laporanId;
  final String muridId;

  LaporanSosial({this.laporanId, required this.muridId});

  @override
  _LaporanSosialState createState() => _LaporanSosialState();
}

class _LaporanSosialState extends State<LaporanSosial> {
  ...
  Future<void> _loadExistingLaporan() async {
    try {
      ...
      if (laporan != null) {
        setState(() {
          _deskripsiController.text = laporan.sosialDeskripsi ?? '';
          _rekomendasiController.text = laporan.sosialRekomendasi ?? '';
          _selectedGrade = laporan.nilaiAnakSosial;
        });
      }
    } catch (e) {
      ...
      try {
        if (widget.laporanId == null) {
          await _service.createLaporanAkademik(
            ...
            sosialDeskripsi: _deskripsiController.text,
            sosialRekomendasi: _rekomendasiController.text,
            nilaiAnakSosial: _selectedGrade,
          );
        } else {
